...
/**
 * Sort a collection of development components topologically wrt. their
...
public class TopoSort {
	...
	/**
	 * Create an instance of the TopoSort class using the given registry/factory
	...
	/**
	 * Sort the development components to be built topologically (determine
	 * build their order).
	 *
	 * @return the result of the topological sort containing the development
	 *         components in build order. The result object will also contain
	...
	/**
	 * Do a topological sort wrt. to dependencies on the given list of items
	 * (wrapper around {@link DevelopmentComponent} to facilitate the removal of
	 * dependencies for the purpose of sorting). Store the result of the sort in
	 * the {@link TopoSortResult}. If there are still items in the list when no
	 * progress can be made on the sort, extract circular dependencies and
	 * report those in the given {@link TopoSortResult} parameter.
	 *
	 * @param topoSortResult
	 *            Collector object for results of the topological sort and
	 ...
	 */
	private void findCircularDependencies(final TopoSortResult topoSortResult,
			final Map<String, ComponentWrapper> itemMap, final List<ComponentWrapper> items) {
		for (final ComponentWrapper item : items) {
			findRecursivelyInUsedDCs(0, item.getComponent(), item, itemMap, topoSortResult,
					new HashSet<DevelopmentComponent>());
		}
	}

	/**
	 * Inspect the given item whether the given component is contained in the
	 * used DCs of the item. Skip the visit if the given item has been visited
	 * or the component is marked as having a circular dependency already .
	 *
	 * @param component
	 *            development component that is to be checked for having
	 *            circular dependencies.
	 * @param parent
	 *            development component (wrapper) that is to be searched for
	 *            usage relations with the given component.
	 * @param itemMap
	 *            name -> item map for lookup of items by name.
	 * @param topoSortResult
	 *            result collector to report circular dependencies to.
	 * @param visitedParents
	 *            collector object to avoid multiple visits on items (and
	 *            infinite recursion aka StackOverflowError).
	 */
	private void findRecursivelyInUsedDCs(final int depth, final DevelopmentComponent component,
			final ComponentWrapper parent, final Map<String, ComponentWrapper> itemMap,
			final TopoSortResult topoSortResult, final Collection<DevelopmentComponent> visitedParents) {
		if (topoSortResult.hasCircularDependency(component)
				|| (parent != null && visitedParents.contains(parent.getComponent()))) {
			return;
		}
	...
	/**
	 * Calculate development components that will need to be rebuilt and build a
	 * mapping of name to item wrapper.
	 *
	 * @param componentsToRebuild
	 *            list of components that should be rebuilt.
	 * @return mapping of name to item wrapper of components (and components
	 *         depending on them) need to be rebuilt.
	 */
	private Map<String, ComponentWrapper> getComponentsToRebuild(
			final Collection<DevelopmentComponent> componentsToRebuild) {
	...
	/**
	 * Create a collection of used DCs from the public part references of the
	 * given component.
	 *
	 * @param component
	 *            the component whose referenced development components are
	 *            requested.
	...
	private Collection<DevelopmentComponent> createListOfUsedDCs(final DevelopmentComponent component) {
		...
		Collection<DevelopmentComponent> usedDCs = new HashSet<DevelopmentComponent>();
	...
	private static final class ComponentWrapper implements Comparable<ComponentWrapper> {
		 ...
		 * @param usedDCs
		 *            components used by this DC so that they don't need to be
		 *            recomputed from the public part reference.
		 */
